...
export function ProjectSettings({ activeProject, onUpdateProject }) { 
  ...
  React.useEffect(() => { 
    if (activeProject) { 
      ...
      setGithubLink(activeProject.github_link || ""); 
  ...
  if (!activeProject) { 
    return ( 
      <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-zinc-900/50 border border-white/5 rounded-[5px]"> 
  ...
  return ( 
    <div className="h-full overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-6 animate-in fade-in duration-500"> 
      <div className="bg-zinc-900/50 border border-white/5 rounded-[5px] p-8 shadow-2xl"> 
        <div className="flex items-center gap-4 mb-8 shrink-0"> 
          <div className="w-12 h-12 rounded-[5px] bg-indigo-500/20 flex items-center justify-center text-indigo-400 text-xl shadow-inner">⚙️</div> 
          <div> 
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8"> 
          <div className="flex flex-col gap-6"> 
            <div className="flex flex-col gap-2"> 
              ...
              <div className="grid grid-cols-3 gap-2 p-1 bg-zinc-950 rounded-[5px] border border-white/5 shadow-inner"> 
            ...
            <div className="flex flex-col gap-2"> 
              ...
              <textarea 
                className="w-full bg-zinc-950 border border-white/5 rounded-[5px] px-4 py-3 text-sm text-zinc-100 focus:border-indigo-500/50 outline-none placeholder:text-zinc-700 transition-all shadow-inner min-h-[120px] resize-none leading-relaxed" 
                value={description} 
          ...
          <div className="flex flex-col gap-6"> 
            <div className="flex flex-col gap-2"> 
              ...
              <div className="flex gap-2"> 
                <input 
                  className="flex-1 bg-zinc-950 border border-white/5 rounded-[5px] px-4 py-3 text-sm text-zinc-100 focus:border-indigo-500/50 outline-none placeholder:text-zinc-700 transition-all shadow-inner font-mono truncate" 
                  value={rootPath} 
                ...
                <button  
                  type="button"  
                  className="px-5 py-3 bg-zinc-800 border border-white/5 rounded-[5px] text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200 transition-all shadow-lg active:scale-95"  
            ...
            <div className="flex flex-col gap-2"> 
              <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 text-indigo-400/80">GitHub Repository URL</label> 
              <input 
                className="w-full bg-zinc-950 border border-white/5 rounded-[5px] px-4 py-3 text-sm text-zinc-100 focus:border-indigo-500/50 outline-none placeholder:text-zinc-700 transition-all shadow-inner font-mono" 
                value={githubLink} 
                onChange={(e) => setGithubLink(e.target.value)} 
                placeholder="https://github.com/username/repo" 
              /> 
              <p className="text-[10px] text-zinc-600 px-1 mt-1">Used by agents as additional context about the project repository.</p> 
            </div> 
          </div> 
          <div className="lg:col-span-2 flex items-center justify-between gap-6 pt-6 border-t border-white/5 mt-4"> 
            ...
            <button  
              type="submit"  
              className="bg-indigo-600 text-white px-10 py-4 text-xs font-black uppercase tracking-[0.2em] rounded-[5px] hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-xl shadow-indigo-600/20 active:scale-95 shrink-0" 
              disabled={saving} 
            > 
